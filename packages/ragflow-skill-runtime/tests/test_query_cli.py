...
""".strip(),
    ...
    def test_fallback_test_can_write_json_and_markdown(self) -> None:
        ...
        self.assertEqual(payload["runtime_partial_failure"]["summary"]["skipped_count"], 2)
        self.assertEqual(file_payload["runtime_partial_failure"]["schema"], "ragflow_runtime_partial_failure_report_v1")
        self.assertIn("RAGFlow Query Fallback Test Report", markdown)
        self.assertIn("runtime_partial_failure_status: `partial`", markdown)
        self.assertIn("runtime_skipped: `2`", markdown)
