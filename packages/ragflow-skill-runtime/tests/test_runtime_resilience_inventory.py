...
class RuntimeResilienceInventoryTests(unittest.TestCase):
    ...
    def test_inventory_classifies_representative_runtime_surfaces(self) -> None:
        ...
        self.assertEqual(refresh_report["status"], "not_applicable")
        self.assertEqual(refresh_report["features"], [])

        parameter_audit = by_command["ragflow-kb-build parameter-audit"]
        self.assertEqual(parameter_audit["status"], "not_applicable")
        self.assertEqual(parameter_audit["features"], [])
        ...
        self.assertEqual(finalize_review["status"], "not_applicable")
        self.assertEqual(finalize_review["features"], [])

        table_evidence = by_command["ragflow-canonical-review table-evidence"]
        self.assertEqual(table_evidence["status"], "not_applicable")
        self.assertEqual(table_evidence["features"], [])
