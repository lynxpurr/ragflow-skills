...
class PlatformSmokeMatrixTests(unittest.TestCase):
    ...
    def test_run_smoke_matrix_for_strict_vendor_env_profile(self) -> None:
        ...
        self.assertIn("doc-to-md split manifest", check_names)
        ...
        self.assertIn("kb benchmark import attribution", check_names)
        self.assertIn("kb benchmark import selection", check_names)
        self.assertIn("kb benchmark preflight redaction", check_names)
        self.assertIn("kb benchmark sample redaction", check_names)
        self.assertIn("kb benchmark sample attribution", check_names)
        self.assertIn("kb benchmark sample selection provenance", check_names)
        ...
        self.assertTrue(
            any(path.endswith("doc_split_manifest.json") for path in profile["artifacts"]),
            profile["artifacts"],
        )
        self.assertTrue(
        ...
        self.assertTrue(
            any(path.endswith("optimization_command_manifest.json") for path in profile["artifacts"]),
            profile["artifacts"],
        )
        self.assertTrue(
        ...
        self.assertTrue(
            any(path.endswith("query_agentic_answer_request_redaction.json") for path in profile["artifacts"]),
            profile["artifacts"],
        )
        self.assertTrue(
            any(path.endswith("query_agentic_answer_review_redaction.json") for path in profile["artifacts"]),
            profile["artifacts"],
        )
        self.assertTrue(
