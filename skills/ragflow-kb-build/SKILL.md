...
python scripts/build.py optimize --execute --doc-manifest ./handoff/doc_manifest.json --kb-name kb:example --profile ./profiles/default.json --benchmark-manifest ./run/benchmark/manifest.json --run-id reviewed-run --confirm-live-build --confirm-kb-name kb:example --confirm-run-id reviewed-run --output ./run/optimization_execute_plan.json
...
- Use `optimize --execute` only after reviewing the command manifest; it currently builds selected disposable candidate KBs and writes retained candidate `kb_manifest.json` files. It requires `--confirm-live-build`, exact `--confirm-kb-name`, and exact `--confirm-run-id`. Benchmark validation and cleanup execution remain separate follow-up steps.
