...
- Use `optimize --plan-only` to load candidate profiles, resolve benchmark artifacts, lint candidates, and plan disposable KB experiment names without creating or deleting anything in RAGFlow; add `--command-manifest-output` to write a dry-run review manifest of the future disposable build, validation, diagnostic, and cleanup-preview commands. Disposable build commands remain disabled and require a future explicit `--execute` path.
