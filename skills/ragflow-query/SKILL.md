...
- `--mode agentic --host-assisted` builds a deterministic `ragflow_agentic_plan_v1`, executes bounded sub-query retrieval, and returns evidence plus `ragflow_agentic_trace_v1` cost/latency data for host synthesis. It does not call an LLM or synthesize an answer.
