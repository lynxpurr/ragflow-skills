...
- 先识别 MinerU 执行模式：本机已安装 MinerU CLI 时可使用 `ragflow-doc-to-md --backend mineru-cli`，或保持 `auto` 让它优先发现本地 CLI；`mineru` / `mineru-agent` 支持 MinerU Agent API（`/parse/file` 创建任务、上传文件、轮询任务、下载 Markdown）；`mineru-sync` / `mineru-local` 支持同步 multipart `/parse` 服务。默认 MinerU 可能是本机 CLI、内网远程服务、VPN、HTTPS gateway 或在线服务，不要把部署位置和协议混为一谈。
- 不确定 MinerU 协议时，不要把配置文件里的 `doc_to_md.backend` 从 `auto` 改成 `mineru`。本机 CLI 可保持 `auto` 或使用 `mineru-cli`，确认 Agent API 后用 `mineru`，确认同步 multipart `/parse` 后用 `mineru-sync`。
...
- 如果本机 MinerU CLI 可用，优先使用 ragflow-doc-to-md --backend auto 或 --backend mineru-cli，并确认输出 Markdown
