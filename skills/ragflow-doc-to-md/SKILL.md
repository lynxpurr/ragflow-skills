...
- `runtime_report.json` records conversion/pipeline stage timings, cold/warm MinerU execution context, process/remote attempts, timeout/resource failure classifications, and measurable asset materialization timing. Use `--runtime-report-md` for a Markdown copy rendered from the same JSON payload.
