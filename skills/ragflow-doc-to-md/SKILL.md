...
python scripts/convert.py postprocess --doc-manifest ./handoff/doc_manifest.json --profile pandoc-epub --output ./handoff-clean
```
...
- Use `postprocess` with profiles `none`, `safe`, `ocr`, `pandoc-epub`, `chunk-markers`, `chunk-markers-conservative`, `chunk-markers-dense`, or `chunk-markers-ragflux-like` when Markdown needs deterministic cleanup before ingestion. Use `pandoc-epub` for pandoc-generated EPUB Markdown before KB build; it conservatively removes marker-only fenced div lines, empty generated anchors, inline style attributes, and generated heading anchor tails while preserving visible text and image references. `chunk-markers` remains the conservative compatibility alias; `chunk-markers-dense` adds page/table/image boundaries, and `chunk-markers-ragflux-like` also adds list boundaries for migration comparison. Use `--output` for non-destructive writes; `--write` is required for in-place rewrites.
