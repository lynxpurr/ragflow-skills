...
The `mineru-cli` backend runs `mineru -b <backend> -p <source> -o <temp-output>` and reads the produced Markdown. The `mineru` backend uses the Agent parsing API shape: create a parse task, upload the local file to the returned signed URL, poll the task, and download the returned Markdown URL. Use `--backend mineru-v4` for MinerU v4 platform-compatible precision APIs, where `--table-quality high` selects `--mineru-v4-model-version vlm` unless `MinerU-HTML` was explicitly configured. Use `--backend mineru-sync` for synchronous multipart `/parse` services.
