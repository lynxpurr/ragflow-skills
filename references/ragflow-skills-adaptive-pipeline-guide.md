...
**日期**：2026-07-06
**版本**：基于 `ragflow-skills` adaptive pipeline reports and MinerU v4 platform-compatible backend support
...
如果使用 MinerU v4 platform-compatible precision API，将 `--backend mineru-fastapi`
替换为 `--backend mineru-v4` 或别名 `--backend mineru-platform`。不要把 v4
platform-compatible URL 配给 `mineru-fastapi`；FastAPI 使用 `/tasks` 协议，v4
platform-compatible protocol 使用 `/api/v4/file-urls/batch`、预签名上传 URL 和
...
- `recommendation.mineru_v4_model_version`：仅适用于 MinerU v4 platform-compatible protocol；高质量表格通常为 `vlm`
...
- 用户显式传入 `--backend mineru-v4` / `mineru-platform` 时，`adaptive` 会保留该
  platform-compatible backend，并推荐 `table_quality: high` 与
