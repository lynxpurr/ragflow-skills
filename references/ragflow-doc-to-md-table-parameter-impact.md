...
在当前 Blackwell + MinerU 3.2.1 FastAPI + `pipeline` 后端环境下，**通过调整 ragflow-doc-to-md 调用参数来显著改善扫描 PDF 表格质量的空间很有限**。表格质量的上限主要由 MinerU 后端模型决定，而非 doc-to-md 包装层参数。若使用 MinerU v4 platform-compatible API，`--table-quality high` 对应的是 `model_version=vlm`，不是 FastAPI 的 `hybrid-auto-engine`。
...
使用 MinerU v4 platform-compatible API 时，命令形态是：
...
这里的 `https://mineru.net` 是官方公共端点示例；其他在线或网关服务只要实现同一
`/api/v4/file-urls/batch`、预签名上传、`/api/v4/extract-results/batch/{batch_id}` 和
`full_zip_url` 合约，也应使用同一个 `mineru-v4` 后端。
...
则保留 v4 platform-compatible backend，并推荐 `mineru_v4_model_version: vlm`。已有
...
| 核心决定层 | `--mineru-v4-model-version` | v4 compatible yes | `vlm` 是 v4 高质量表格路径；`MinerU-HTML` 显式配置会被保留 |
...
3. 是否想尝试更高精度？ → 先确认 GPU 是否支持 `hybrid-auto-engine`（Blackwell 不支持）
   - 若使用 MinerU v4 platform-compatible API，改为确认是否接受 `model_version=vlm` 的在线解析路径。
